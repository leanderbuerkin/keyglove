...
// int8_t rotation_x, rotation_y, rotation_z, temperature;

void setup_accelerometer_gyroscope(int sensor_address)
{
  Wire.begin();

  Wire.beginTransmission(sensor_address); // before every Wire.write()
  Wire.write(POWER_MANAGMENT_REGISTER_ADDRESS);
  Wire.write(WAKE_UP_MESSAGE_TO_POWER_MANAGMENT_REGISTER);
  Wire.endTransmission(true);             // after every Wire.write()
}
...
void read_accelerometer(int sensor_address)
{
  ...
  Wire.endTransmission(false); // after every Wire.write()
  Wire.requestFrom(sensor_address, MINIMAL_NUMBER_OF_DATA_REGISTERS);
