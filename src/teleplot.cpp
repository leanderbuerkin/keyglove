...
#define WAKE_UP_MESSAGE_TO_POWER_MANAGEMENT 0
...
void setup(void)
{
    ...
    mpu1.setHighPassFilter(MPU6050_HIGHPASS_DISABLE);
    mpu2.setHighPassFilter(MPU6050_HIGHPASS_DISABLE);
...
void serial_print_sensor_data(TwoWire &I2C_bus, int mpu_address)
{
    ...
    // use static_cast<int8_t> for more continuous values, without it is faster
